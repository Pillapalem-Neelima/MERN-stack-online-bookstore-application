...
function Products() {
  ...
  useEffect(() => {
    axios
      .get(`http://localhost:5000/item`, { withCredentials: true })
      .then((response) => {
    ...
    if (user) {
      axios
        .get(`http://localhost:5000/wishlist/${user.id}`, { withCredentials: true })
        .then((response) => {
      ...
      <div className="container mx-auto p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 justify-items-center">
          {items.map((item) => (
            ...
            >
              <img
              src={`http://localhost:5000/${item.itemImage.replace(/\\/g, "/")}`}
              alt={item.title}
